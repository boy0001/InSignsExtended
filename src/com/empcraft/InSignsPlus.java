...
public final class InSignsPlus extends JavaPlugin implements Listener {
    ...
    public ArrayDeque<SignPlus> queue = new ArrayDeque<SignPlus>();
    public HashMap<PlayerSign, SignPlus> updateMap = new HashMap<PlayerSign, SignPlus>();
    ...
    public String getmsg(String key) {
        String msg = LANGUAGE.getString(key);
        if (msg == null) {
            return "";
        }
        return colorise(msg);
    ...
    public void addUpdateQueue(PlayerSign sign, SignPlus sp) {
        if (!updateMap.containsKey(sign)) {
            updateMap.put(sign, sp);
            queue.add(sp);
        }
    }
    
    public void addUpdateQueue(Player player, Location loc, boolean[] lines) {
        SignPlus sp = new SignPlus(loc, player, lines);
        PlayerSign sign = new PlayerSign(player.getName(), loc);
        addUpdateQueue(sign, sp);
    }
    ...
    public void onEnable() {
        ...
        InSignsPlus.LANGUAGE = YamlConfiguration.loadConfiguration(yamlFile);
        // end lang //
        ...
        BUFFER = getConfig().getInt("signs.autoupdate.buffer") * 4;
        AMOUNT = getConfig().getInt("signs.autoupdate.updates-per-tick");
            ...
            Runnable runnable = new Runnable() {
                ...
                public void run() {
                    ...
                    for (int i = 0; i < Math.min(AMOUNT, size); i++) {
                        SignPlus sp = queue.poll();
                        boolean result = sp.update();
                        PlayerSign ps = new PlayerSign(sp.getPlayerName(), sp.getLocation());
                        if (result) {
                            queue.add(sp);
                        }
                        else {
                    ...
                    for (Chunk chunk : chunks) {
                        for (Entity entity : chunk.getEntities()) {
                            if (entity instanceof Player) {
                                if (myplayers.contains((Player) entity) == false) {
        ...
        if (event.getAction() == Action.RIGHT_CLICK_BLOCK || event.getAction() == Action.LEFT_CLICK_BLOCK) {
            ...
            if (block != null) {
                ...
                if (state instanceof Sign) {
                    ...
                    PlayerSign ps = new PlayerSign(sp.getPlayerName(), sp.getLocation());
                    SignPlus current = updateMap.get(ps) ;
                    if (current != null) {
